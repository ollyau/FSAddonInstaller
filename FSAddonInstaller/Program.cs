...
namespace FSAddonInstaller {
    class Program {
        ...
        /* String constants */
        private static string WELCOME_MESSAGE = "Generic FSX Addon Installer/Uninstaller\r\nBy Orion Lyau\r\n\r\nUsage:\r\n[directory]\t- Installs files from the specified directory into FSX.\r\n\t\t- Generates an XML file for uninstallation.\r\n[*.xml]\t\t- Triggers uninstall based on XML configuration file.\r\n\r\n";
        private static string EXIT_MESSAGE = "\r\nPress any key to close...";
        private static string INVALID_DIR_OR_FILE = "ERROR: An invalid directory or file was passed as an argument.";
        private static string INVALID_NUM_ARGS = "ERROR: Invalid number of arguments.";
        private static string XML_OUTPUT = "\r\n\r\n\tSource directory: {0}\r\n\tDestination directory: {1}\r\n\tTime stamp: {2}\r\n\tProcessed: {3} files, {4} directories\r\n\tSkipped {5} file(s).\r\n\tBacked up {6} file(s).\r\n\r\n";
        private static string FSX_REG = @"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\microsoft games\flight simulator\10.0";

        /* Program members */
        private static State state;
        private static string fsxDirectory = null;
        private static int directoryCount = 0;
        private static int fileCount = 0;
        private static int skipCount = 0;
        private static int backupCount = 0;
        private static XDocument xmlConfig;
        private static bool backupAndReplaceEnabled = true;
        ...
        static void Main(string[] args) {
            ...
            if (args.Length != 1) {
                ...
                state = State.INSTALL;
                run(args[0]);
            } else if (File.Exists(args[0]) && args[0].EndsWith(".xml")) {
                state = State.UNINSTALL;
                run(args[0]);
            } else {
                Console.WriteLine(INVALID_DIR_OR_FILE);
            }
        ...
        static void uninstall(string path) {
            XmlDocument input;
            XmlNodeList nodes;
